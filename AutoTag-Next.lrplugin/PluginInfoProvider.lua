...
-- Helper: Metadata & Context Section
local function getMetadataSection(f, prefs)
...
-- Helper: AI Configuration Section
local function getAISection(f, prefs)
    return {
...
-- Helper: Prompt Customization Section
local function getPromptSection(f, prefs)
    ...
    return {
        ...
        f:row {
            f:static_text { title = "Plantilla de Descripción:", width = LrView.share('label_width'), alignment = 'right' },
            f:edit_field {
                value = LrView.bind { key = 'descriptionTemplate', bind_to_object = prefs },
                width = 400,
                placeholder = "Ej: {institucion} - {actividad} en {lugar}"
            }
        },
        f:static_text {
            title = "Variables disponibles: {institucion}, {area}, {actividad}, {lugar}, {contexto}",
            font = "<system/small>",
            alignment = 'right',
            width = LrView.share('label_width')
        }
...
-- Helper: Batch Processing Section
local function getBatchSection(f, prefs)
    return {
...
-- Helper: Metadata Control Section
local function getMetadataControlSection(f, prefs)
    return {
...
function PluginInfoProvider.sectionsForTopOfDialog(f, propertyTable)
    ...
    if prefs.descriptionTemplate == nil then prefs.descriptionTemplate = "" end
    ...
    return {
        getMetadataSection(f, prefs),
        getAISection(f, prefs),
        getPromptSection(f, prefs),
        getBatchSection(f, prefs),
        getMetadataControlSection(f, prefs)
    }
end
